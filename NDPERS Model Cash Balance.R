...
FileName <- 'Model Inputs Cash Balance.xlsx'
#
#Reading Input File
...
##################################################################################################################################################################
#
#This is payroll section. it needs to be done separately since the payroll values for previous years are references in NC and EE calcs later on and the i-1 shift
for(i in StartIndex:length(FYE)){
  ...
  NewHireCBPayroll[i] <- (TotalPayroll[i] - CurrentPayroll[i])*NewHireCBPct
  ...
  ProjectedNewHiresCBPayroll[i-1] <- NewHireCBPayroll[i]
}
#
##################################################################################################################################################################
...
RunModel <- function(AnalysisType, SimReturn, SimVolatility, ER_Policy, ScenType, LvDollarorPercent){
  ...
  for(i in StartIndex:length(FYE)){
    ...
    NewHireBenPct[i] <- 0.25/100*(FYE[i] - StartProjectionYear)
    BenPayments_NewHires[i] <- -1*NewHireBenPct[i]*(NewHireDBPayroll[i] + NewHireDCPayroll[i])
    BenPayments_Total[i] <- BenPayments_Total[i-1]*(1 + max(0.00806 - 0.00266*(FYE[i] - StartProjectionYear), BenGrowthMax))
    ...
    #
    #Cash Balance
    TempValue <- 0.5 + min(0.02*(FYE[i]-NC_StaryYear),0.5)*(1-LumpSumTakeRate)
    CB_ActivePct[i] <- min(0.5/TempValue,1)
    CB_StartingAAL[i] <- CB_TotalAAL[i-1]*(1+NewDR_CurrentHires[i-1]) + (CB_TotalNC[i-1] + BenPaymentsCB[i])*(1+NewDR_CurrentHires[i-1])^0.5
    GL_Growth <- 100*(Actual_ICR[i] - Interest_Credit)
    CB_GainLoss[i] <- CB_StartingAAL[i]*CB_ActivePct[i]*((1+GL_Factor)^GL_Growth - 1)
    CB_TotalAAL[i] <- CB_StartingAAL[i] + CB_GainLoss[i]
    CB_TotalNC[i] <- NC_CB*NewHireCBPayroll[i]
    
    Actual_ICR[i] <- 0.06
    if(dis_r_CB == dis_r_newHires){
      PayCredit[i] <- PayCredit_rate*NewHireCBPayroll[i] 
    }
    
    if(FYE[i] <= 2022){
      BP_Pct_AAL[i] <- BenPaymentPoint1
    } else if(FYE[i] <= 2024){
      BP_Pct_AAL[i] <- max(BenPaymentPoint2,BP_Pct_AAL[i-1] - 0.025)
    } else if(FYE[i] == 2025){
      BP_Pct_AAL[i] <- max(BenPaymentPoint2,BP_Pct_AAL[i-1] - 0.015)
    } else if(FYE[i] == 2040){
      BP_Pct_AAL[i] <- max(BenPaymentPoint2,BP_Pct_AAL[i-1] - 0.01)
    } else {
      BP_Pct_AAL[i] <- min(BenPaymentPoint3,BP_Pct_AAL[i-1] + 0.01)
    }
    
    BenPaymentsCB[i] <- -BP_Pct_AAL[i]*(CB_TotalAAL[i]*(1+NewDR_CurrentHires[i]) + CB_TotalNC[i]*(1+NewDR_CurrentHires[i])^0.5)
    AcctBal[i] <- PayCredit[i-1] + CBInterest[i-1] + BenPaymentsCB[i] + AcctBal[i-1]
    CBInterest[i] <- Actual_ICR[i]*AcctBal[i] + ((1+Actual_ICR[i])^0.5 - 1)*(PayCredit[i] + BenPaymentsCB[i])
    #
    #NC, EE Contrib
...
ScenarioPlot <- function(Data, YAxisLabel){
  Scenarios <- c('Assumption','Model','Recession','Recurring Recession')
  ggplot(Data, aes(x = FYE), col = colnames(Data)) +
    ...
    labs(y = YAxisLabel, x = 'Year', col = Scenarios) + ggtitle(YAxisLabel) + theme(legend.position = 'bottom')
